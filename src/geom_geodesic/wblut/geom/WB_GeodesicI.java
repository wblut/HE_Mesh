...
class WB_GeodesicI {
	...
	private void createMesh() {
		...
		if (div == EQUALCHORD) {
			...
			for (int i = 1; i < v; i++) {
				refpoints[i - 1 + 2 * (v - 1)] = apices[0].mulAddMul(1.0 - i * iv, i * iv, apices[1]);
				refpoints[i - 1 + 2 * (v - 1)].normalizeSelf();
				refpoints[i - 1 + v - 1] = apices[0].mulAddMul(1.0 - i * iv, i * iv, apices[2]);
				refpoints[i - 1 + v - 1].normalizeSelf();
				refpoints[i - 1] = apices[1].mulAddMul(1.0 - i * iv, i * iv, apices[2]);
			...
			for (int i = 1; i < v; i++) {
				refpoints[i - 1 + 2 * (v - 1)] = gf
						.createPoint(WB_Geodesic.getPointOnGreatCircleArc(apices[0], apices[1], i * iv));
				refpoints[i - 1 + v - 1] = gf
						.createPoint(WB_Geodesic.getPointOnGreatCircleArc(apices[0], apices[2], i * iv));
				refpoints[i - 1] = gf.createPoint(WB_Geodesic.getPointOnGreatCircleArc(apices[1], apices[2], i * iv));
			}
		}
		...
		final List<WB_Point> zeropoints = new WB_PointList();
		points.add(apices[0]);
		for (int i = 0; i < v - 1; i++) {
		...
		case TETRAHEDRON:
			...
			T = new WB_Transform3D().addRotateZ(Math.PI).addRotateY(Math.PI / 180.0 * 250.5288);
			for (final WB_Point p : zeropoints) {
				points.add(T.applyAsPoint(p));
			}
			break;
		case OCTAHEDRON:
			final List<WB_Point> points4 = new WB_PointList();
		...
		default:
			...
			final List<WB_Point> points5 = new WB_PointList();
			for (final WB_Point p : zeropoints) {
