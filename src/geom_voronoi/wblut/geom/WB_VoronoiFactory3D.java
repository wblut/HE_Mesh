...
public class WB_VoronoiFactory3D extends WB_VoronoiFactory2D {
	...
	public static int[][] getVoronoi3DNeighbors(final WB_CoordCollection points) {
		...
		final List<wblut.external.ProGAL.Point> tmppoints = new ArrayList<>(nv);
		final WB_KDTreeInteger3D<WB_Coord> tree = new WB_KDTreeInteger3D<>();
		WB_Coord p;
		for (int i = 0; i < nv; i++) {
			p = points.get(i);
	...
	public static int[][] getVoronoi3DNeighborPairs(final WB_CoordCollection points) {
		...
		if (nv == 2) {
			...
			return new int[][] { { 0, 1 }, { 0, 2 }, { 0, 3 }, { 1, 2 }, { 1, 3 }, { 2, 3 } };
		}
		final List<wblut.external.ProGAL.Point> tmppoints = new ArrayList<>(nv);
		final WB_KDTreeInteger3D<WB_Coord> tree = new WB_KDTreeInteger3D<>();
		WB_Coord p;
		...
		final int[][] pairs = new int[numberOfPairs / 2][2];
		int index = 0;
		for (int i = 0; i < nv; i++) {
		...
		final int[][] finalPairs = new int[index][2];
	...
	public static WB_Network getNetwork(final WB_CoordCollection points) {
		...
		final WB_KDTreeInteger3D<WB_Coord> tree = new WB_KDTreeInteger3D<>();
		final List<WB_Coord> vorPoints = new WB_CoordList();
		int index = 0;
		WB_Point vorPoint;
		for (final CTetrahedron tetra : tetrahedra) {
		...
		for (final CTriangle triangle : triangles) {
			...
			if ((tetra2 == null) || (tetra2.containsBigPoint())) {
				continue;
			}
		...
		final int[][] finalPairs = new int[pairs.size()][2];
	...
	public static WB_Voronoi3D getVoronoi3DBruteForce(final WB_CoordCollection points, int nv, final WB_AABB aabb,
		...
		for (int i = 0; i < nv; i++) {
			...
			boolean unique;
			final ArrayList<WB_Plane> cleaned = new ArrayList<>();
	...
	public static void main(final String[] args) {
		...
		final WB_Point[] points = new WB_Point[numpoints];
		for (int i = 0; i < 11; i++) {
			for (int j = 0; j < 1; j++) {
				points[i] = new WB_Point(-200 + i * 40, -200 + j * 40, 0);
			}
		}
		final HEMC_VoronoiCells multiCreator = new HEMC_VoronoiCells();
		multiCreator.setPoints(points);
		multiCreator.setContainer(container);
		multiCreator.setOffset(5);
		final List<HE_Mesh> cells = (multiCreator.create()).toList();
		cells.size();
	}
}
