...
public class WB_GeometryFactory2D {
	...
	public WB_CoordinateSystem createCSFromOriginAndXY2D(final WB_Coord origin, final WB_Coord X, final WB_Coord Y) {
		...
		if (WB_Epsilon.isZeroSq(lX.cross(lY).getSqLength3D())) {
	...
	public List<WB_Point> createIntersectionPoints2D(final WB_Line L, final WB_Circle C) {
		...
		final double c = WB_Vector.getSqLength3D(C.getCenter()) + WB_Vector.getSqLength3D(L.getOrigin())
	...
	public List<WB_Polygon> createBoundaryPolygons2D(final Collection<? extends WB_Polygon> poly) {
		...
		if (poly.size() < 1) {
			...
			final Iterator<? extends WB_Polygon> pItr = poly.iterator();
	...
	public List<WB_Circle> createCirclePPC(final WB_Coord p, final WB_Coord q, final WB_Circle C) {
		...
		// Both points outside
			...
			final WB_Line ABbis = createBisector2D(p, q);
			if (WB_Vector.isCollinear(C.getCenter(), ABbis.getOrigin(), ABbis.getPoint(100.0))) {
			...
			final List<WB_Circle> iresult = new ArrayList<>();
			final WB_Circle iC;
			final WB_Circle iC2;
			final double k2;
			...
			if (WB_Vector.isCollinear(iC2.getCenter(), ABbis.getOrigin(), ABbis.getPoint(100.0))) {
		...
		if (WB_GeometryOp2D.classifyPointToCircle2D(p, C) == WB_Classification.ON) {
			...
			if (tangentcircle) {
				...
				if (WB_GeometryOp2D.classifyPointToLine2D(p, L) == WB_GeometryOp2D.classifyPointToLine2D(C.getCenter(),
					...
					p1 = createInversionPoint2D(WB_Point.addMul(C.getCenter(), C.getRadius(), WB_VectorFactory.X()),
							inversion);
					p2 = createInversionPoint2D(WB_Point.addMul(C.getCenter(), -C.getRadius(), WB_VectorFactory.X()),
							inversion);
	...
	public List<WB_Circle> createCircleCCC(WB_Circle C1, WB_Circle C2, WB_Circle C3) {
		...
		final List<WB_Circle> circles = new ArrayList<>();
	...
	public List<WB_Circle> createCircleTangentToLineAndCircle(final WB_Line L, final WB_Circle C, final double r) {
		...
		final List<WB_Coord> intersections = new ArrayList<>();
