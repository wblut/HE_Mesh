...
public class HEC_Grid extends HEC_Creator {
	...
	private HE_Mesh createGrid() {
		final int U = getU();
		final int V = getV();
		final double sizeU = getSizeU();
		final double sizeV = getSizeV();
		final double valueScale = getValueScale();
		if (sizeU == 0 || sizeV == 0) {
			return new HE_Mesh();
		}
		...
		fl.setVertices(points).setUVW(uvw).setFaces(faces).setCheckDuplicateVertices(false).setCheckNormals(false);
		final HE_Mesh baseMesh = fl.createBase();
	...
	private HE_Mesh createStepGrid() {
		...
		for (int i = 0; i < U; i++) {
			for (int j = 0; j < V; j++) {
				final WB_Point p1 = new WB_Point(i * dU, j * dV, valueScale * getValue(i, j, outOfRange));
				final WB_Point p2 = new WB_Point((i + 1) * dU, j * dV, valueScale * getValue(i, j, outOfRange));
				final WB_Point p3 = new WB_Point((i + 1) * dU, (j + 1) * dV, valueScale * getValue(i, j, outOfRange));
				final WB_Point p4 = new WB_Point(i * dU, (j + 1) * dV, valueScale * getValue(i, j, outOfRange));
		...
		if (base) {
			for (int i = 0; i < U; i++) {
				for (int j = 0; j < V; j++) {
					final WB_Point p1 = new WB_Point(i * dU, j * dV, valueScale * getBaseValue());
					final WB_Point p2 = new WB_Point((i + 1) * dU, j * dV, valueScale * getBaseValue());
					final WB_Point p3 = new WB_Point((i + 1) * dU, (j + 1) * dV, valueScale * getBaseValue());
					final WB_Point p4 = new WB_Point(i * dU, (j + 1) * dV, valueScale * getBaseValue());
		...
		for (int i = (base ? -1 : 0); i < (base ? U : U - 1); i++) {
			for (int j = 0; j < V; j++) {
				final List<WB_Coord> points = new WB_CoordList();
		...
		final HE_Mesh baseMesh = fq.createBase();
