...
public class WB_SNoise implements WB_Noise { // Simplex noise in 2D, 3D and 4D
	...
	public double value2D(final double x, final double y) {
		...
		final double x2 = (x0 - 1.0) + (2.0 * G2); // Offsets for last corner in (x,y)
		// unskewed coords
	...
	public double value3D(final double x, final double y, final double z) {
		final double xin = sx * x;
		final double yin = sy * y;
		final double zin = sz * z;
		double n0, n1, n2, n3; // Noise contributions from the four corners
		// Skew the input space to determine which simplex cell we're in
		final double s = (xin + yin + zin) * F3; // Very nice and simple skew factor
		// for 3D
		final int i = fastfloor(xin + s);
		final int j = fastfloor(yin + s);
		...
		final double x2 = (x0 - i2) + (2.0 * G3); // Offsets for third corner in (x,y,z)
		// coords
		final double y2 = (y0 - j2) + (2.0 * G3);
		final double z2 = (z0 - k2) + (2.0 * G3);
		final double x3 = (x0 - 1.0) + (3.0 * G3); // Offsets for last corner in (x,y,z)
		// coords
	...
	public double value4D(final double x, final double y, final double z, final double w) {
		...
		final double yin = sy * y;
		final double zin = sz * z;
		final double win = sw * w;
		double n0, n1, n2, n3, n4; // Noise contributions from the five corners
		// Skew the (x,y,z,w) space to determine which cell of 24 simplices
		// we're in
		final double s = (xin + yin + zin + win) * F4; // Factor for 4D skewing
		final int i = fastfloor(xin + s);
		final int j = fastfloor(yin + s);
		final int k = fastfloor(zin + s);
		final int l = fastfloor(win + s);
		...
		final double x2 = (x0 - i2) + (2.0 * G4); // Offsets for third corner in (x,y,z,w)
		// coords
		final double y2 = (y0 - j2) + (2.0 * G4);
		final double z2 = (z0 - k2) + (2.0 * G4);
		final double w2 = (w0 - l2) + (2.0 * G4);
		final double x3 = (x0 - i3) + (3.0 * G4); // Offsets for fourth corner in
		// (x,y,z,w) coords
		final double y3 = (y0 - j3) + (3.0 * G4);
		final double z3 = (z0 - k3) + (3.0 * G4);
		final double w3 = (w0 - l3) + (3.0 * G4);
		final double x4 = (x0 - 1.0) + (4.0 * G4); // Offsets for last corner in (x,y,z,w)
		// coords
