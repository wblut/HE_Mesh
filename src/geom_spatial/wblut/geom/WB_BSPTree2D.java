...
public class WB_BSPTree2D {
	...
	private void partitionCoincidentSegments(final WB_BSPNode2D node, final WB_Segment S, final List<WB_Segment> pos,
		...
		for (int i = 0; i < node.segments.size(); i++) {
			...
			for (int j = 0; j < partSegments.size(); j++) {
				...
				double[] intersection;
				if (u0 < u1) {
					intersection = intervalIntersection(u0, u1, Math.min(v0, v1), Math.max(v0, v1));
					if (intersection[0] == 2) {
						final WB_Point pi = L.getPoint(intersection[1]);
						final WB_Point pj = L.getPoint(intersection[2]);
						if (u0 < intersection[1]) {
							newpartSegments.add(new WB_Segment(thisS.getOrigin(), pi));
						}
						coSame.add(new WB_Segment(pi, pj));
						if (u1 > intersection[2]) {
							newpartSegments.add(new WB_Segment(pj, thisS.getEndpoint()));
						}
					} else {// this segment doesn't coincide with an edge
						newpartSegments.add(thisS);
					}
				} else {
					intersection = intervalIntersection(u1, u0, Math.min(v0, v1), Math.max(v0, v1));
					if (intersection[0] == 2) {
						final WB_Point pi = L.getPoint(intersection[1]);
						final WB_Point pj = L.getPoint(intersection[2]);
						if (u1 < intersection[1]) {
							newpartSegments.add(new WB_Segment(pi, thisS.getEndpoint()));
						}
						coDiff.add(new WB_Segment(pj, pi));
						if (u0 > intersection[2]) {
							newpartSegments.add(new WB_Segment(thisS.getOrigin(), pj));
						}
					} else {
						newpartSegments.add(thisS);
					}
	...
	private WB_BSPNode2D negate(final WB_BSPNode2D node) {
		...
		for (int i = 0; i < node.segments.size(); i++) {
			...
			negNode.segments.add(new WB_Segment(seg.getEndpoint(), seg.getOrigin()));
		}
		if (node.pos != null) {
	...
	private WB_IntersectionResult closestPoint2D(final WB_Segment S, final WB_Line L) {
		...
		if (i.t2 <= WB_Epsilon.EPSILON) {
			i.t2 = 0;
			i.object = new WB_Segment(((WB_Segment) i.object).getOrigin(), new WB_Point(S.getOrigin()));
		...
		if (i.t2 >= S.getLength() - WB_Epsilon.EPSILON) {
			i.t2 = 1;
			i.object = new WB_Segment(((WB_Segment) i.object).getOrigin(), new WB_Point(S.getEndpoint()));
			i.sqDist = ((WB_Segment) i.object).getLength();
			i.sqDist *= i.sqDist;
			i.intersection = false;
		}
