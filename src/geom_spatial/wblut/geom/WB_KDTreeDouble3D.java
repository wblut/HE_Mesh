 ...
 * @param <T>
 */
public class WB_KDTreeDouble3D<T extends WB_Coord> {
	/**  */
	 ...
	 * @param binsize
	 */
	public WB_KDTreeDouble3D(final int binsize) {
		this._dim = 3;
	...
	private class WB_KDNodeDouble<T extends WB_Coord> {
		...
		private double lookup(final WB_Coord point) {
			for (int i = 0; i < _binSize; i++) {
				if (WB_Epsilon.isZeroSq(WB_GeometryOp.getSqDistance3D(point, _bin[i].coord))) {
					return _bin[i].value;
				}
			}
			return Double.NaN;
		...
		private void findNearest(final QueryResultDouble<T> heap, final WB_Coord data) {
			...
			if (_isLeaf) {
				for (int i = 0; i < _binSize; i++) {
					final double dist = WB_GeometryOp.getSqDistance3D(_bin[i].coord, data);
		...
		private WB_KDEntryDouble<T>[] range(final WB_Coord center, final double r2) {
			...
			int n = 0;
			for (int i = 0; i < _binSize; i++) {
				final double d2 = WB_GeometryOp.getSqDistance3D(center, _bin[i].coord);
				if (d2 <= r2) {
					_bin[i].d2 = d2;
					tmp[n++] = _bin[i];
				}
