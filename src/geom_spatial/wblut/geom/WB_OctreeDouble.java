...
public class WB_OctreeDouble {
	...
	public boolean remove(final WB_Coord p) {
		...
		if (node != null) {
			for (final WB_OctreeDoubleEntry eo : node.entries) {
	...
	public List<WB_OctreeDoubleEntry> getEntriesInRange(final WB_AABB AABB) {
		...
		if (box.intersects(AABB)) {
			if (entries != null) {
				...
				for (int i = 0; i < 8; i++) {
					if (nodes[i] != null) {
						final List<WB_OctreeDoubleEntry> points = nodes[i].getEntriesInRange(AABB);
						if (points != null) {
	...
	public List<WB_OctreeDoubleEntry> getEntriesInRange(final WB_Sphere sphere) {
		...
		if (box.intersects(sphere)) {
			if (entries != null) {
				...
				for (int i = 0; i < 8; i++) {
					if (nodes[i] != null) {
						final List<WB_OctreeDoubleEntry> points = nodes[i].getEntriesInRange(sphere);
						if (points != null) {
							result.addAll(points);
						}
					}
	...
	public double[] getValues() {
		...
		int i = 0;
		for (final WB_OctreeDoubleEntry eo : allEntries) {
			result[i++] = eo.value;
		}
		return result;
	}
	...
	public double[] getValuesInRange(final WB_AABB AABB) {
		...
		int i = 0;
		for (final WB_OctreeDoubleEntry eo : entriesInRange) {
			result[i++] = eo.value;
		}
		return result;
	}
