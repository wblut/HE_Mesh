...
public class WB_QuadtreeInteger {
	...
	public List<WB_QuadtreeInteger> getNodes() {
		final List<WB_QuadtreeInteger> result = new FastList<>();
		if (numNodes > 0) {
			for (int i = 0; i < 4; i++) {
				if (nodes[i] != null) {
					result.add(nodes[i]);
	...
	public List<WB_QuadtreeIntegerEntry> getEntriesInRange(final WB_AABB2D AABB) {
		FastList<WB_QuadtreeIntegerEntry> result = new FastList<>();
	...
	public List<WB_QuadtreeIntegerEntry> getEntriesInRange(final WB_Circle circle) {
		final FastList<WB_QuadtreeIntegerEntry> result = new FastList<>();
