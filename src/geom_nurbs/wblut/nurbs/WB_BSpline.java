...
public class WB_BSpline implements WB_Curve {
	...
	public WB_BSpline refineKnot(final WB_NurbsKnot K) {
		...
		for (final double[] element : multVal) {
			final int orig = knot.multiplicity(element[0]);
			final int target = (int) element[1];
			final int rep = Math.max(0, target - orig);
			for (int j = 0; j < rep; j++) {
				newVal.add(element[0]);
			}
		}
		if (newVal.size() == 0) {
			return this;
		}
		final double[] fX = new double[newVal.size()];
		...
		for (int j = r; j >= 0; j--) {
			...
			for (int el = 1; el <= p; el++) {
				...
				if (WB_Epsilon.isZero(alpha)) {
					...
					Q[ind - 1] = gf.createInterpolatedPoint(Q[ind], Q[ind - 1], alpha);
				}
			}
			Ubar.setValue(k, X[j]);
			k--;
		...
		while (b < m) {
			...
			if (r > 0) {
				...
				for (j = 1; j <= r; j++) {
					...
					final int sj = mul + j;
					for (k = p; k >= sj; k--) {
						bpts[k] = gf.createInterpolatedPoint(bpts[k - 1], bpts[k], alfs[k - sj]);
					}
				...
				for (int tr = 1; tr < oldr; tr++) {
					...
					while (j - i > tr) {
						if (i < cind) {
							...
							Q[i] = gf.createInterpolatedPoint(Q[i - 1], Q[i], alf);
						}
						if (j >= lbz) {
							if (j - tr <= kind - ph + oldr) {
								final double gam = (ub - Uh.value(j - tr)) / den;
								ebpts[kj] = gf.createInterpolatedPoint(ebpts[kj + 1], ebpts[kj], gam);
							} else {
								ebpts[kj] = gf.createInterpolatedPoint(ebpts[kj + 1], ebpts[kj], bet);
							}
						}
						i++;
						j--;
		...
		for (int k = 1; k <= d; k++) {
			...
			for (int i = 0; i <= r - k; i++) {
				PK[k][i] = WB_Point.sub(PK[k - 1][i + 1], PK[k - 1][i])
						.mulSelf(tmp / (knot.value(r1 + i + p + 1) - knot.value(r1 + i + k)));
			}
		}
		return PK;
	}
