...
public class WB_RBezierSurface extends WB_BezierSurface {
	...
	public WB_RBezierSurface elevateUDegree() {
		...
		for (int j = 0; j <= m; j++) {
			...
			for (int i = 1; i <= n; i++) {
				npoints[i][j] = WB_PointHomogeneous.interpolate(wpoints[i][j], wpoints[i - 1][j], i * inp);
			}
		}
	...
	public WB_RBezierSurface elevateVDegree() {
		...
		for (int i = 0; i <= n; i++) {
			...
			final double inp = 1.0 / (n + 1);
			for (int j = 1; j <= m; j++) {
				npoints[i][j] = WB_PointHomogeneous.interpolate(wpoints[i][j], wpoints[i][j - 1], j * inp);
			}
		}
		return new WB_RBezierSurface(npoints);
	}
