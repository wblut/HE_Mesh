...
public class WB_SDF {
	...
	public static double fLineSegment(final WB_Coord p, final WB_Coord a, final WB_Coord b) {
		...
		final double t = saturate(dot3D(WB_Vector.sub(p, a), ab) / dot3D(ab, ab));
	...
	public static double fCone(final WB_Coord p, final double radius, final double height) {
		...
		final WB_Vector tip = q.sub(new WB_Vector(0, height));
		final WB_Vector mantleDir = normalize(new WB_Vector(height, radius));
		final double mantle = dot2D(tip, mantleDir);
		double d = max(mantle, -q.yd());
		final double projected = dot2D(tip, new WB_Vector(mantleDir.yd(), -mantleDir.xd()));
		// distance to tip
	...
	public static double fOpUnionColumns(final double a, final double b, final double r, final double n) {
		if (a < r && b < r) {
			final WB_Vector p = new WB_Vector(a, b);
			final double columnradius = r * sqrt(2) / ((n - 1) * 2 + sqrt(2));
			pR45(p);
