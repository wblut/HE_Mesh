...
public class HEMC_Danzer extends HEMC_MultiCreator {
	...
	void create(final HE_MeshCollection result) {
		...
		final WB_Danzer3D danzer = new WB_Danzer3D(getType(), getScale(), getOrigin());
		final WB_Point shift = getOrigin().sub(danzer.getTiles().get(0).getCenter());
		danzer.inflate(getLevel());
		final HE_Mesh[] tmpCells = new HE_Mesh[danzer.getNumberOfTiles()];
		int id = 0;
