...
public class HEC_FromSurface extends HEC_Creator {
	...
	public HEC_FromSurface setRange(final double lowerU, final double upperU, final double lowerV,
			final double upperV) {
		final WB_Surface surf = getSurface();
	...
	public HE_Mesh createBase() {
		final int U = getU();
		final int V = getV();
		final double lowerU = getLowerU();
		final double upperU = getUpperU();
		final double lowerV = getLowerV();
		final double upperV = getUpperV();
		final WB_Surface surf = getSurface();
		final boolean uWrap = getUWrap();
		final boolean vWrap = getVWrap();
		if (surf == null) {
			return new HE_Mesh();
		}
