...
public class WB_GeometryOp2D extends WB_GeometryOpGLU {
	...
	public static final WB_IntersectionResult getIntersection2D(final WB_Segment S1, final WB_Segment S2) {
		final double a1 = WB_GeometryOp2D.twiceSignedTriArea2D(S1.getOrigin(), S1.getEndpoint(), S2.getEndpoint());
		final double a2 = WB_GeometryOp2D.twiceSignedTriArea2D(S1.getOrigin(), S1.getEndpoint(), S2.getOrigin());
		if (!WB_Epsilon.isZero(a1) && !WB_Epsilon.isZero(a2) && a1 * a2 < 0) {
			final double a3 = WB_GeometryOp2D.twiceSignedTriArea2D(S2.getOrigin(), S2.getEndpoint(), S1.getOrigin());
	...
	public static final double getSqDistance2D(final WB_Coord p, final WB_PolyLine PL) {
		...
		double d2;
		for (int s = 0; s < PL.getNumberSegments(); s++) {
			d2 = WB_GeometryOp2D.getSqDistance2D(p, PL.getSegment(s));
			if (d2 < d2min) {
				d2min = d2;
			}
		}
	...
	public static final WB_Classification classifyPointToCircle2D(final WB_Coord p, final WB_Circle C) {
		final double dist = WB_GeometryOp2D.getDistanceToPoint2D(p, C.getCenter());
	...
	public static final WB_Classification classifyCircleToCircle2D(final WB_Circle C1, final WB_Circle C2) {
		...
		final double dist = WB_GeometryOp2D.getDistanceToPoint2D(C1.getCenter(), C2.getCenter());
	...
	public static final boolean pointInTriangle2D(final WB_Coord p, final WB_Coord A, final WB_Coord B,
			final WB_Coord C) {
		if (WB_Epsilon.isZeroSq(WB_GeometryOp2D.getSqDistanceToLine2D(A, B, C))) {
			return false;
		}
	...
	public static final boolean isCollinear2D(final WB_Coord o, final WB_Coord p, final WB_Coord q) {
		if (WB_Epsilon.isZeroSq(WB_GeometryOp2D.getSqDistanceToPoint2D(p, q))) {
			return true;
		}
		if (WB_Epsilon.isZeroSq(WB_GeometryOp2D.getSqDistanceToPoint2D(o, q))) {
			return true;
		}
		if (WB_Epsilon.isZeroSq(WB_GeometryOp2D.getSqDistanceToPoint2D(o, p))) {
			return true;
		}
		return WB_Epsilon.isZeroSq(WB_GeometryOp2D.getSqDistanceToLine2D(o, p, q));
	}
