...
public class WB_GeometryOp3D extends WB_GeometryOp2D {
	...
	public static WB_IntersectionResult getIntersection3D(final WB_Sphere S, final WB_Plane P) {
		final double d = WB_GeometryOp3D.getDistance3D(S.getCenter(), P);
		...
		if (WB_Epsilon.isZero(d - radius)) {
			...
			i.t1 = radius;
			i.t2 = 0;
			i.object = WB_GeometryOp3D.getClosestPoint3D(S.getCenter(), P);
			i.dimension = 0;
			i.sqDist = 0;
			return i;
		} else {
			...
			final WB_IntersectionResult i = new WB_IntersectionResult();
			final WB_Point p = WB_GeometryOp3D.getClosestPoint3D(S.getCenter(), P);
			i.intersection = true;
			i.t1 = d;
			i.t2 = 0;
	...
	public static WB_IntersectionResult getIntersection3D(final WB_Sphere S, final WB_Circle C) {
		...
		final WB_IntersectionResult is = getIntersection3D(S, P);
		if (!is.intersection) {
			return NOINTERSECTION();
		}
		if (is.dimension == 0) {
	...
	public static WB_IntersectionResult getIntersection3D(final WB_Sphere S1, final WB_Sphere S2, final WB_Sphere S3) {
		final WB_IntersectionResult is = getIntersection3D(S1, S2);
		if (!is.intersection) {
			return NOINTERSECTION();
		}
		if (is.dimension == 0) {
	...
	public static WB_Coord[] getFourthPoint(final WB_Coord p1, final double d1, final WB_Coord p2, final double d2,
		...
		final WB_IntersectionResult is = getIntersection3D(S1, S2, S3);
		if (!is.intersection) {
