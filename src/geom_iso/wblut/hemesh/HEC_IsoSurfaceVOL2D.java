...
public class HEC_IsoSurfaceVOL2D extends HEC_Creator {
	...
	public static void main(final String[] args) {
		...
		final float[][] values = new float[resx + 1][resy + 1];
		final WB_Segment[] segs = new WB_Segment[num];
		for (int i = 0; i < num; i++) {
			final WB_Point p1 = new WB_Point(Math.random() * resx, Math.random() * resy);
			final WB_Point p2 = new WB_Point(Math.random() * resx, Math.random() * resy);
		...
		final HEC_IsoSurfaceVOL2D iso = new HEC_IsoSurfaceVOL2D().setResolution(resx, resy).setValues(values)
				.setSize(dx, dy).setIsolevel(0.5, 1.5);
		iso.setCenter(center);
		final HE_Mesh mesh = new HE_Mesh(iso);
		mesh.validate();
	}
}
